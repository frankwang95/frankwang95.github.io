...
$$L_p(T) = \int_0^T f_x x_p + f_p + \lambda_p h(x, x_t) + \lambda [h_x x_p + h_{\dot x} x_{t,p} + h_p] \, d\tau+ \mu g_x x_p |_0$$
...
L_p(T) &= &&\int_0^T f_x x_p + f_p + \lambda_p h + \lambda [h_x x_p + h_p ] - \lambda_t  x_p \, d\tau + \\
          &&\mu g_x x_p |_0 + \lambda x_p |_T - \lambda  x_p |_0 \\
       &= \int_0^T f_p + \lambda_p h + \lambda h_p + x_p[f_x + \lambda h_x - \lambda_t] \, d\tau + \\
          &&x_p (\mu g_x - \lambda) |_0+ x_p \lambda |_T
\end{align}$$
...
2. The $$x_p\lambda |_T$$ term at the end of the equation can eliminated with an initial value condition $$\lambda(T) = 0$$.
3. Finally, the $$x_p(\mu g_x - \lambda) |_0$$ term disappears by setting $$\mu = g_x^{-1}\lambda = \lambda$$ though it is worth noting that applying this theorem requires no explicit consideration of the $$\mu$$ term of the Lagrangian.
